...
Writes the string `content` into file on `path`. Throws an error if the file does not exist on given `path`.
...
Appends the string `content` into file on `path`. Throws an error if the file does not exist on given `path`.
...
Moves the file on `oldPath` to `newPath`. Throws an error if the operation fails.
...
Deletes the file on `path`. Throws an error if the operation fails.
