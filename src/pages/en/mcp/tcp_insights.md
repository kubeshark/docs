...
## How This Differs from L4 Flows

The [L4 Tools](/en/mcp/l4_tools) provide real-time flow data (who talks to whom, how many bytes, TCP handshake RTT) **without requiring dissection**. Their TCP handshake RTT percentiles come from eBPF programs that hook into the kernel's `sock/inet_sock_set_state` tracepoint — the kernel itself tracks when each socket enters `SYN_SENT` or `SYN_RECV` and when it reaches `ESTABLISHED`, giving you handshake timing with zero packet inspection overhead. Percentiles are computed in userspace using t-digest over a sliding 2-minute window.

**TCP Expert Insights require dissection.** When dissection is active, Kubeshark's workers run a `TCPMetricsTracker` on every TCP connection that **processes each individual TCP packet** as it passes through the reassembly pipeline. For each packet, it inspects sequence numbers, acknowledgment numbers, TCP flags, window sizes, and timestamps — tracking retransmissions, detecting out-of-order delivery, measuring per-packet RTT from data-to-ACK timing, and computing jitter as the standard deviation of RTT samples. This is the same deep analysis Wireshark performs when you open a pcap file.

| | L4 Flows | TCP Expert Insights |
|--|----------|-------------------|
| **Requires dissection** | No | Yes |
| **Data source** | eBPF `sock/inet_sock_set_state` tracepoint + socket byte counters | Per-packet TCP analysis in the dissection reassembly pipeline |
| **How it works** | Kernel reports socket state transitions; userspace aggregates via t-digest | `TCPMetricsTracker` inspects every TCP packet for seq/ack numbers, flags, window sizes, and timing |
| **What you get** | Bytes, packets, rates, handshake RTT percentiles (P50/P90/P99) | Full TCP health: retransmissions, window problems, per-packet RTT min/max/avg, jitter, connection lifecycle, goodput |
| **Queried via** | `list_l4_flows` | `list_api_calls` with `kfl=tcp_flow` |
| **Overhead** | Minimal (kernel counters, no packet inspection) | Moderate (per-packet inspection during dissection) |
| **Wireshark equivalent** | Statistics > Conversations | Analyze > Expert Information |

---

## How TCP Metrics Are Computed

When dissection is enabled, each Kubeshark worker runs a `TCPMetricsTracker` per TCP connection inside the packet reassembly pipeline. As packets flow through:

1. **Every TCP packet is inspected** — the tracker reads sequence numbers, acknowledgment numbers, TCP flags (SYN, ACK, FIN, RST), window size, and TCP options (window scale)
2. **Retransmissions are detected** by comparing each segment's sequence number against the expected next sequence — if `seq < next_expected_seq` with payload, it's a retransmission. If 3+ duplicate ACKs preceded it, it's a fast retransmission
3. **RTT is measured per data segment** — when a data packet is sent, its sequence number and timestamp are recorded. When the corresponding ACK arrives, RTT = `ack_time - send_time`. Up to 100 recent samples are kept for min/max/avg/jitter calculation
4. **Window metrics are tracked** by monitoring the advertised window in each ACK. Zero-window events (window=0) and window-full events (bytes-in-flight >= advertised window) are counted
5. **Connection lifecycle** is tracked via a bitmask: SYN, SYN-ACK, ACK, DATA, FIN, RST — matching Wireshark's TCP Conversation Completeness
6. **Handshake timing** is measured from SYN to SYN-ACK (initial RTT) and SYN to ACK (full handshake)

The tracker emits a `TcpMetrics` snapshot attached to each `ConnBaseEntry` (per-connection) and `FlowBaseEntry` (aggregated flow), which is then available through the MCP `list_api_calls` endpoint.
