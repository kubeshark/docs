(function() {
  ...
  class AiEmbed {
    ...
    startTeaserCycle(questions) {
      if (!questions || questions.length === 0) return;
      let idx = 0;
      const show = () => {
        if (this.isOpen || !this.teaser) return;
        this.teaser.textContent = questions[idx];
    ...
    async loadAgentConfig() {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          if (!this.isInline) {
            this.startTeaserCycle(this.agentConfig.opening_questions || []);
          }
        }
      } catch (err) {
        console.error('[aiembed] Failed to load agent config:', err);
      } finally {
        // Always reveal inline panel — even on error, don't leave a blank area
        if (this.isInline) {
          this.panel.classList.add('loaded');
        }
      }
    }
